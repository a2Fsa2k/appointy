/*
  Doctors.jsx — Browse all doctors with specialty filter.
  URL: /doctors or /doctors/:speciality (e.g., /doctors/Dermatologist)

  TRICK: The same component handles BOTH URLs.
  - If speciality param exists → filter doctors by that specialty
  - If no param → show all doctors

  The filter is client-side filtering (not a new API call).
  All doctors are already loaded via AppContext (getDoctorsData).
  Filtering happens in JavaScript: doctors.filter(doc => doc.speciality === speciality)

  Toggle behavior on each specialty: clicking the same one again
  navigates to /doctors (clears filter). This creates a toggle:
  click on = filter by specialty, click again = clear filter.
*/
...
const Doctors = () => {
  ...
  // Re-filter when doctors list loads or specialty URL param changes
    ...
    <div>
      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
        ...
        <div className={`flex-col gap-4 text-sm text-gray-600 ${showFilter ? 'flex' : 'hidden sm:flex'}`}>
          {/*
            Each specialty is a clickable filter. The onClick toggles:
            if already filtered by this specialty → go to /doctors (clear)
            if not → go to /doctors/<specialty> (apply filter)
          */}
        ...
        <div className='w-full grid grid-cols-auto gap-4 gap-y-6'>
          {filterDoc.map((item, index) => (
            // Click navigates to the appointment booking page for this doctor
            <div onClick={() => { navigate(`/appointment/${item._id}`); scrollTo(0, 0) }} className='border border-[#C9D8FF] rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
              <img className='bg-[#EAEFFF]' src={item.image} alt="" />
              <div className='p-4'>
                {/* Green dot = available, gray dot = not available */}
